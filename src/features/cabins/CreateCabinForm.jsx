...
function CreateCabinForm({cabinToEdit = {}, onCloseModal}) {
  ...
  function onSubmit(data){
    const image = typeof data.image === "string" || !data.image ? data.image : data.image[0];
    ...
    <Form onSubmit={handleSubmit(onSubmit, onError)} type={onCloseModal ? "modal" : "regular"}>
      ...
      <FormRow label="Discount" error={errors?.discount?.message}>
        <Input type="number" id="discount" defaultValue={0} disabled={isWorking} {...register("discount", {
          required: "This field is required",
          validate: (value)=> value <= getValues().regularPrice || "Discount should be smaller than price"
        })}/>
      </FormRow>
