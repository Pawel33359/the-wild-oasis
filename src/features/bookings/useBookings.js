...
export function useBookings(){
  ...
  // const filters = !filterValue || filterValue === "all" ? null : [{field: "status", value: filterValue, method: "eq"}];
  ...
  const {isLoading, data: {data: bookings, count} = {}, error} = useQuery({
    ...
    queryFn: ()=>getBookings({filter, /*filters,*/ sortBy, page})
  });
  ...
  if(page < pageCount) queryClient.prefetchQuery({
    ...
    queryFn: ()=>getBookings({filter, /*filters,*/ sortBy, page:page + 1})
  })
